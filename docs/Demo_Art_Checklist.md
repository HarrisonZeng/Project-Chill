...
| # | Item | Status | Notes |
|---|---|---|---|
| D1 | A natural way *into* the call each launch (no title screen — it is a call app) | ✅ live | Launching the app is placing the call; **Yua answers by herself** (1.4 s ring on first launch, 0.7 s returning, then connect + fade). No tap. Web audio still unlocks on the first click (C5) and the intro never waits for it. `call_intro` scenario asserts the no-button flow. |
| D2 | UI chrome matches the room | ✅ live (B, 2026-09-10) | Owner picked **B cozy journal** from four Codex mockups on the real frame, with amendments: no type box unless Type Mode is on (except the name and task asks), icon-only Settings and Log, tasks panel as wide as the timer, nothing decorative on the timer. Theme-level: `data/theme/project_chill.tres` + `assets/art/ui/icons/*.svg`. `chrome` scenario photographs the open states. |
