...
| # | Item | Status | Notes |
|---|---|---|---|
| A1 | Room layout | ✅ original restored · ⬜ wider gap | **Owner verdict 2026-08-19: frontal repaint rejected.** Keep the ORIGINAL (3/4 angle, U-desk). Original is live again with all newer layers on it. The one wanted change — **the gap inside the U made larger** — needs a reference-guided image edit, which Codex's CLI tool cannot do (text-to-image only; see §F). **🅾 Owner: run the prompt below in ChatGPT with `room_original.png` attached, save as `assets/art/backgrounds/room_master.png`** — Claude derives the keyed layers programmatically (glass and near-desk masks carry over unchanged) and wires it. |
...
| A2 | Room lighting follows the view outside — no daylight rays at night | 🔧 redo after A1 | The pipeline is proven (night = lamp-lit EDIT of the master; glass mask 0.00% diff, desk 1%; Yua + hands modulated cool at night, warm at sunset) but the night art was made for the rejected frontal room. Re-run on the U-desk master once A1 lands. Sunset stays a warm tint on day art unless it reads flat. |
...
- **THE REAL BOUNDARY (learned 2026-08-19): Codex's image tool is text-to-image only — it cannot take a reference image as input.** So it can (a) generate a brand-new picture from words, and (b) do *programmatic* pixel edits of an existing file (paint magenta over regions, move/copy pixels) — which is how every 0.0000 "edit" was actually made. It **cannot** repaint part of an existing picture (relight, move a counter, change a face) with the rest held fixed. Anything that needs that is a job for a reference-guided editor: **ChatGPT with GPT Image (attach the file), or the OpenAI `images/edits` API with a mask** — the owner runs it, Claude wires the result.
