...
| # | Item | Status | Notes |
|---|---|---|---|
| A1 | Room layout | 🔧 redo in progress | **Owner verdict 2026-08-19: the frontal single-desk repaint is rejected.** Keep the ORIGINAL — 3/4 angle (she's sideways to the viewer, less eye-to-eye pressure) and the U-shaped desk (room for gadgets). Only change: **the gap inside the U made larger** (window counter pushed back, floor visible). Codex is editing `room_original.png` → new `room_master` + glass/desk keyed edits. Yua anchor restored to the original 0.68. |
...
| B2b | **Stance art was mislabelled** — "at her screen" was showing the eyes-closed variant | 🔧 fixing | The real at-screen image (`5d8adf5f`, head turned to her monitor) is installed as `yua_at_work_keyed`; it is a different pose so it needs its OWN hands cut — Codex running. The eyes-closed image is a valid at_player "resting" variant, kept aside. |
