const DESC := "Ep6 (脑子飞了, was Ep3 before v12): real 6-session unlock, confession → platform question; chips, typed keywords, AI-off cover, remembered answer, P.S. ending."
...
## jumping straight to ep06_01 sets ep06_seen, and relaunch() saves before it
## wipes — so a direct jump would make the episode look already-seen.

func _open_ask(g) -> void:
	g.game._show_node("ep06_ask")
	await g.settle()
...
func run(g) -> void:
	# Progression: the sixth completed focus opens Ep6 for real, and it plays out.
	await g.click_yua()
	await g.play_forward()
	for i in range(5):
		await g.complete_focus()
		g.check_node("session %d → Ep%d" % [i + 1, i + 1], "ep%02d_01" % (i + 1))
		await g.play_forward()
	await g.complete_focus()
	g.check_node("sixth session → Ep6", "ep06_01")
	g.check("Ep6 opens with the 战力 confession", g.full_line().contains("霸王龙和剑齿象"), g.full_line())
	g.check("ep06_seen is set when Ep6 opens", bool(g.flags().get("ep06_seen", false)), str(g.flags()))
	await g.click_card()
	g.check_node("继续 → the platform question", "ep06_ask")
	...
	g.check_node("B 站 → ep06_bili", "ep06_bili")
	...
	await g.click_card()
	g.check_node("继续 → ending", "ep06_end")
	...
	g.check_node("typed 油管 → ep06_yt", "ep06_yt")
	g.check("typed platform remembered", _remembered(g) == "youtube", _remembered(g))
	await g.click_card()
	g.check_node("油管 → 继续 → ending", "ep06_end")

	await _open_ask(g)
	await g.type_reply("不刷，我不怎么看这些")
	g.check_node("typed 不刷 → ep06_none", "ep06_none")

	await _open_ask(g)
	await g.type_reply("Bilibili 和小红书都刷")
	g.check_node("first matching rule wins (B 站 before 小红书)", "ep06_bili")

	await _open_ask(g)
	await g.type_reply("主要打游戏")
	g.check_node("typed 游戏 → ep06_game", "ep06_game")
	...
	g.check_node("别的 → ep06_other", "ep06_other")
	g.check("ep06_other keeps a click-through (no dead end)", g.choices().size() == 1, str(g.choices()))
	await g.type_reply("Reddit")
	g.check_node("unknown platform with AI off → scripted cover", "ep06_any")
	g.check("unknown answer remembered verbatim", _remembered(g) == "Reddit", _remembered(g))
	await g.click_card()
	g.check_node("cover → 继续 → ending", "ep06_end")
	...
	g.check("ep06_seen survives a restart", bool(g.flags().get("ep06_seen", false)), str(g.flags()))
	await g.click_yua()
	await g.play_forward()
	await g.complete_focus()
	g.check("Ep6 does not replay on the next session", g.node_id() != "ep06_01", g.node_id())
